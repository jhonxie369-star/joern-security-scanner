...
// CWE-94: SpEL注入规则 - 移除复杂防护检测
object SpelInjectionRule {
  ...
  def sinkNode(cpg: io.shiftleft.codepropertygraph.Cpg, kind: String, safeCalls: List[io.shiftleft.codepropertygraph.generated.nodes.Call]) = {
    (
      ...
      safeCalls.filter(c =>
        c.methodFullName.contains("SpelExpressionParser") && c.name == "parseExpression"
      ) ++

      // Expression.getValue
      safeCalls.filter(c =>
        c.methodFullName.contains("Expression") && c.name == "getValue"
      ) ++

      // ExpressionParser相关
      safeCalls.filter(c =>
        c.methodFullName.contains("ExpressionParser") && Set("parseExpression", "parseRaw").contains(c.name)
      )
    ).l
  }
  ...
  def detect(cpg: io.shiftleft.codepropertygraph.Cpg, sources: List[io.shiftleft.codepropertygraph.generated.nodes.AstNode], safeCalls: List[io.shiftleft.codepropertygraph.generated.nodes.Call]): List[String] = {
    ...
    val sinks = sinkNode(cpg, "spel-injection", safeCalls)
    ...
    groupedFlows.map { flow =>
      ...
      JsonlHelper.formatFinding(
        ...
        message = "SpEL注入风险: 用户输入未经验证直接用于SpEL表达式",
        sinkCode = sink.code,
        dataflowPath = path
      )
    }.toList

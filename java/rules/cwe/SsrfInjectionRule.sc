...
// CWE-918: SSRF规则 - 移除复杂防护检测
object SsrfInjectionRule {
  ...
  def sinkNode(cpg: io.shiftleft.codepropertygraph.Cpg, kind: String, safeCalls: List[io.shiftleft.codepropertygraph.generated.nodes.Call]) = {
    (
      // URL.openConnection
      safeCalls.filter(c =>
        c.methodFullName.contains("URL") && c.name == "openConnection"
      ) ++

      // HttpURLConnection
      safeCalls.filter(c =>
        c.methodFullName.contains("HttpURLConnection") && Set("connect", "getInputStream").contains(c.name)
      ) ++

      // Apache HttpClient
      safeCalls.filter(c =>
        c.methodFullName.contains("HttpClient") && c.name == "execute"
      ) ++

      // OkHttp
      safeCalls.filter(c =>
        c.methodFullName.contains("OkHttpClient") && c.name == "newCall"
      ) ++

      // RestTemplate
      safeCalls.filter(c =>
        c.methodFullName.contains("RestTemplate") && Set("getForObject", "postForObject", "exchange").contains(c.name)
      )
    ).l
  }
  ...
  def detect(cpg: io.shiftleft.codepropertygraph.Cpg, sources: List[io.shiftleft.codepropertygraph.generated.nodes.AstNode], safeCalls: List[io.shiftleft.codepropertygraph.generated.nodes.Call]): List[String] = {
    ...
    val sinks = sinkNode(cpg, "ssrf", safeCalls)
    ...
    groupedFlows.map { flow =>
      ...
      JsonlHelper.formatFinding(
        ...
        message = "SSRF风险: 用户输入未经验证直接用于HTTP请求",
        sinkCode = sink.code,
        dataflowPath = path
      )
    }.toList

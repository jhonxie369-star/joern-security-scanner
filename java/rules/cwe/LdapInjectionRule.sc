...
object LdapInjectionRule {
  ...
  def sinkNode(cpg: io.shiftleft.codepropertygraph.Cpg, kind: String, safeCalls: List[io.shiftleft.codepropertygraph.generated.nodes.Call]) = {
    (
      // DirContext.search
      safeCalls.filter(c =>
        c.methodFullName.contains("DirContext") && c.name == "search"
      ) ++

      // LdapContext.search
      safeCalls.filter(c =>
        c.methodFullName.contains("LdapContext") && c.name == "search"
      ) ++

      // InitialDirContext.search
      safeCalls.filter(c =>
        c.methodFullName.contains("InitialDirContext") && c.name == "search"
      ) ++

      // SearchControls相关
      safeCalls.filter(c =>
        c.methodFullName.contains("SearchControls") && c.name == "setSearchScope"
      )
    ).l
  }
  ...
  def detect(cpg: io.shiftleft.codepropertygraph.Cpg, sources: List[io.shiftleft.codepropertygraph.generated.nodes.AstNode], safeCalls: List[io.shiftleft.codepropertygraph.generated.nodes.Call]): List[String] = {
    ...
    val sinks = sinkNode(cpg, "ldap-injection", safeCalls)
    ...
    groupedFlows.map { flow =>
      ...
      JsonlHelper.formatFinding(
        ...
        message = "LDAP注入风险: 用户输入未经验证直接用于LDAP查询",
        sinkCode = sink.code,
        dataflowPath = path
      )
    }.toList
